...
// AGREGADO PARA IMPLEMENTAR JOIN Y MULTICOLAS DE PRIORIDAD
    ...
    bool joinFlag;	// Especifica si se va 
			// a hacer join sobre él

    int prioridad; // Va a guardar la prioridad del thread actual
    int originalP; // Va a guardad la prioridad original con la que se creo el thread
...
// FIN DE AGREGADO
  ...
  public:
    ...
    Thread(const char* debugName, int p); 	//inicializa un thread con prioridad p
...
// ##############################################################
//
// Esto es de la práctica 3
//
// AGREGADO PARA IMPLEMENTAR JOIN Y MULTICOLAS DE PRIORIDAD
//

    int getPriority() { return prioridad;}
    void SetPriority(int prio) { prioridad = prio; }
    int GetOriginalPriority() { return originalP; }
    void Join(Thread *);
    bool askedByJoin() {return joinFlag;}
...
// AGREGADO NUESTRO
    ...
    OpenFileId AgregarDescriptor(OpenFile *);
    OpenFile *GetDescriptor(OpenFileId);
    void BorrarDescriptor(OpenFileId);
#endif
// ####################################################
