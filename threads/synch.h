...
//AGREGADO
    List<Thread*> *verCola() {return queue;}
//
...
class Lock {
  ...
  private:
    const char* name;				// para depuraci�n
    Semaphore * lock; // a�adir aqu� otros campos que sean necesarios
    Thread * owner;
};
...
class Condition {
  public:
    ...
    void Broadcast(Lock *conditionLock);// the crrurrentThread for all of 
					// these operations
